...
def get_list_product_recommend(list_item_id):
    arr_product_recommend = []
    for i in range(0, len(list_item_id)):
        mycursor.execute("SELECT * FROM products where id="+str(list_item_id[i]))
        myresult = mycursor.fetchall()
        for x in myresult:
            arr_product_recommend.append(x)
    return arr_product_recommend
...
def hello_world(id):
  ...
  arr_product_recommend = get_list_product_recommend(arr_item_recommend)
  print(arr_product_recommend)
  return json.dumps(arr_product_recommend)
